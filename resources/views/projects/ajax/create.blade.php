                ...
                <div class="row p-20">
                    <div class="col-lg-3 col-md-6">
                        <x-forms.text class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.projects.projectName')"
                            ...
                            :fieldPlaceholder="__('placeholders.project')" :fieldValue="$projectTemplate->project_name ?? ''" />
                    </div>
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="project_category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option @if ($projectTemplate && ($projectTemplate->category_id == $category->id)) selected @endif value="{{ $category->id }}">{{ ucwords($category->category_name) }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="client_id" id="client_id"
                                    ...
                                    @foreach ($clients as $client)
                                        <option @if (request()->has('default_client') && request('default_client') == $client->id) selected @endif
                    ...
                    <div class="col-md-12 col-lg-6">
                        <div class="form-group my-3">
                            ...
                            <textarea name="project_summary" id="project_summary-text" class="d-none">{!! $projectTemplate->project_summary ?? '' !!}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 col-lg-6">
                        <div class="form-group my-3">
                            ...
                            <textarea name="notes" id="notes-text" class="d-none">{!! $projectTemplate->notes ?? '' !!}</textarea>
                        </div>
                    </div>
                                ...
                                <select class="form-control multiple-users" multiple name="user_id[]"
                                    ...
                                    @foreach ($employees as $item)
                                        <option @if (request()->has('default_assign') && request('default_assign') == $item->id) selected @endif
                                            @if (isset($projectTemplateMembers) && in_array($item->id, $projectTemplateMembers))
                                                selected
                                            @endif
