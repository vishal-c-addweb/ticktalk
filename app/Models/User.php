...
{
    use Notifiable, EntrustUserTrait, Authenticatable, Authorizable, CanResetPassword, HasFactory;
     ...
     */
    protected $fillable = [
        'name', 'email', 'password', 'mobile', 'login', 'status', 'image', 'gender', 'locale', 'onesignal_player_id', 'email_notifications', 'country_id','rocket_token', 'rocket_refresh_token', 'rocket_uid', 'rocket_username'
    ];
    ...
    {
        if (!is_null($exceptId)) {
            $users = User::with('employeeDetail')->withoutGlobalScope('active')
                ->join('role_user', 'role_user.user_id', '=', 'users.id')
                ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ...
            if (!isRunningInConsoleOrSeeding()) {
                if (user()->permission('view_employees') == 'added') {
