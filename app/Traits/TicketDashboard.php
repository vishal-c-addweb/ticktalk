...
trait TicketDashboard
{
    ...
    public function ticketDashboard()
    {
        ...
        $this->totalUnresolvedTickets = Ticket::whereBetween(DB::raw('DATE(`updated_at`)'), [$startDate, $endDate])
            ->where('status', '!=', 'resolved')
            ->where('status', '!=', 'closed')
            ->count();

        $this->totalResolvedTickets = Ticket::whereBetween(DB::raw('DATE(`updated_at`)'), [$startDate, $endDate])
            ->where('status', '=', 'resolved')
            ->orWhere('status', '=', 'closed')
            ->count();

        $this->totalUnassignedTicket = Ticket::whereBetween(DB::raw('DATE(`updated_at`)'), [$startDate, $endDate])
            ->whereNull('agent_id')
            ->count();
