...
class TimelogController extends AccountBaseController
{
    ...
    public function store(StoreTimeLog $request)
    {
        ...
        $startDateTime = Carbon::parse($startDateTime)->setTimezone('UTC');
        ...
        $endDateTime = Carbon::parse($endDateTime)->setTimezone('UTC');
    ...
    public function startTimer(StartTimer $request)
    {
        ...
        if (is_null($activeTimer)) {
            ...
            if ($request->project_id != '') {
                $this->logProjectActivity($request->project_id, 'timerStartedBy');
                $this->logUserActivity($this->user->id, 'timerStartedProject' . ucwords($timeLog->project->project_name));
            }
            else {
                $this->logUserActivity($this->user->id, 'timerStartedTask' . ucwords($timeLog->task->heading));
            }
    ...
    public function stopTimer(Request $request)
    {
        ...
        if (!is_null($timeLog->project_id)) {
            $this->logProjectActivity($timeLog->project_id, 'timerStoppedBy' . ' ' . user()->name);
        }
        ...
        $this->logUserActivity($this->user->id, 'timerStoppedBy' . ' ' . user()->name);
