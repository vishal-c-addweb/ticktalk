...
class CreditNoteController extends AccountBaseController
{
    ...
    public function show($id)
    {
        $this->viewPermission = user()->permission('view_invoices');
        $this->creditNote = CreditNotes::with('invoice')->findOrFail($id);
        abort_403(!($this->viewPermission == 'all' || ($this->viewPermission == 'added' && $this->creditNote->invoice->added_by == user()->id) || ($this->viewPermission == 'owned' && $this->creditNote->client_id == user()->id)));
    ...
    public function applyToInvoice($id)
    {
        ...
        $this->creditNote = CreditNotes::findOrFail($id);
        $this->nonPaidInvoices = Invoice::pending()->where('credit_note', 0)->where('client_id', $this->creditNote->client_id);
